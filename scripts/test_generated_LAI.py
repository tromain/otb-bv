...
class GenerateLaiTest(unittest.TestCase):
    ...
    def test1_parser_muscate_landsat8(self):
        cfg = genLAI.ConfigFile()
        cfg.import_data_from_file(**json.load(open(self.cfgFile)))
    ...
    def test5_img_inversion_muscate_sentinel2A(self):
        cfg = genLAI.ConfigFile()
        cfg.import_data_from_file(**json.load(open(self.cfgFile)))
        prod = genLAI.parse_xml(self.xml_muscate_s2A, 'Muscate_Metadata', None, True)
    ...
    def test5_img_inversion_muscate_landsat8(self):
        cfg = genLAI.ConfigFile()
        cfg.import_data_from_file(**json.load(open(self.cfgFile)))
        prod = genLAI.parse_xml(self.xml_muscate_l8, 'Muscate_Metadata', self.landsat8_rsr, False)
        ...
        for res in self.resolution_l8:
            ...
            image_LAI = self.output_images_folder + 'LANDSAT8-OLITIRS-XS_20200701-102304-097_L2A_T31TFL_C_V2-2_test_result_{}.tif'.format(res)
...
if __name__ == '__main__':
    ...
    for test_name in test_names:
        if "landsat" in test_name or "load_cfg" in test_name:
