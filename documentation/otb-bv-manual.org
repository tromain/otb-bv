...
#+begin_latex
\begin{equation}
f_{APAR} = \frac{a_0+\sum_{i=1}^{N}a_i \rho_i+\sum_{i=1}^{N}\sum_{j=1,j\neq i}^{N}a_{ij} \rho_i\rho_j}{b_0+\sum_{i=1}^{N}b_i \rho_i+\sum_{i=1}^{N}\sum_{j=1,j\neq i}^{N}b_{ij} \rho_i\rho_j}
\end{equation}
#+end_latex

Linear regression is a particular case of nonlinear regression.
